...
class GazePredictorHandler:
    ...
    def __call__(self, img, lms5, undo_roll=False, debug_input=False, *args, **kwargs):
        ...
        if undo_roll:
            ...
            norm = np.linalg.norm(vec_lms5)

            if norm < 1e-6:
                print("[Warning] lms5 landmarks too close — skipping roll compensation.")
                roll = 0.0
            else:
                vec_lms5 /= norm
                roll = np.arccos(np.clip(np.dot(vec_lms5, np.array([1, 0])), -1.0, 1.0))
                roll = -roll * 180 / np.pi
