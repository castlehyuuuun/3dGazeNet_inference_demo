...
def draw_gaze(image, lms, vector, colour=[0, 0, 255]):
    ...
    thickness = int(max(min(diag / 10, 20), 1))
...
def draw_eyes(image, lms, eyes, draw_eyeball=True, draw_iris=True, draw_rings=True, colour=[178, 255, 102]):
    ...
    thickness_iris = int(max(min(diag / 100, 5), 1))
    thickness_eyeball = int(max(min(diag / 100, 5), 1))
    thickness_rings = int(max(min(diag / 100, 3), 1))
